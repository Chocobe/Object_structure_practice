...
// 생성자
	...
	private int days() {
		return Period.between(
				from.toLocalDate(), 
				to.toLocalDate()).plusDays(1).getDays();
	}
	
	
	private List<DateTimeInterval> splitByDay(int days) {
		List<DateTimeInterval> result = new ArrayList<DateTimeInterval>();
	...
	private void addMiddleDays(List<DateTimeInterval> result, int days) {
		for(int loop = 1; loop < (days - 1); loop++) {
			result.add(DateTimeInterval.during(
							from.toLocalDate().plusDays(loop)));
		}
	}
